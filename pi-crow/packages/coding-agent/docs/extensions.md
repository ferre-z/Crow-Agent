...
```typescript
import type { ExtensionAPI } from "@crow/pi-coding-agent";
...
| Package | Purpose |
|---------|---------|
| `@crow/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@crow/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@crow/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@crow/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@crow/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@crow/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@crow/pi-coding-agent";
...
```typescript
import { SessionManager } from "@crow/pi-coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@crow/pi-ai";
...
```typescript
import type { AutocompleteItem } from "@crow/pi-tui";
...
```typescript
import { Box, Text } from "@crow/pi-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@crow/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@crow/pi-ai";
...
```typescript
import { withFileMutationQueue } from "@crow/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@crow/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@crow/pi-coding-agent";
...
```typescript
import { createBashTool } from "@crow/pi-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/crow/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@crow/pi-coding-agent";
...
```typescript
import { Text, Component } from "@crow/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@crow/pi-coding-agent";
import { matchesKey } from "@crow/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@crow/pi-coding-agent";
