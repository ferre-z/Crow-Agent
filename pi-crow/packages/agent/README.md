...
Stateful agent with tool execution and event streaming. Built on `@crow/pi-ai`.

## Installation

```bash
npm install @crow/pi-agent-core
```

## Quick Start

```typescript
import { Agent } from "@crow/pi-agent-core";
...
```typescript
declare module "@crow/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@crow/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@crow/pi-agent-core";
