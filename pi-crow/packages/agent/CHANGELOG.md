...
- Added `AgentToolResult.addedToolNames` propagation to `ToolResultMessage` so tools introduced by a result can be loaded from that transcript point onward ([#6474](https://github.com/crow/pi-mono/pull/6474)).
...
- Added custom metadata support in JSONL session headers ([#6417](https://github.com/crow/pi/pull/6417) by [@ArcadiaLin](https://github.com/ArcadiaLin)).
- Exported `InMemorySessionStorage` and `JsonlSessionStorage` ([#6435](https://github.com/crow/pi/issues/6435)).

### Fixed

- Fixed harness split-turn compaction to serialize summary requests so single-concurrency providers are not asked to run overlapping generations ([#5536](https://github.com/crow/pi/issues/5536)).
- Fixed harness tool calls from length-truncated assistant messages to fail instead of waiting for missing tool results ([#6285](https://github.com/crow/pi/pull/6285)).
- Fixed harness session ingestion to normalize `null` message content before context projection, avoiding crashes on lax imported transcripts ([#6343](https://github.com/crow/pi/pull/6343)).
- Fixed non-positive or oversized harness shell execution timeouts to fail with a clear validation error instead of being clamped to an immediate timeout ([#6181](https://github.com/crow/pi/issues/6181)).
- Fixed harness session storage short entry ids to use the random tail of the generated uuidv7 instead of the timestamp prefix, which was nearly constant between calls ([#6242](https://github.com/crow/pi/issues/6242)).
...
- Removed the `@crow/pi-agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.

### Fixed

- Fixed harness session names to normalize newline characters before storing labels ([#5999](https://github.com/crow/pi/pull/5999) by [@haoqixu](https://github.com/haoqixu)).
- Fixed harness compaction estimates to ignore malformed all-zero assistant usage after truncated responses ([#5526](https://github.com/crow/pi/pull/5526) by [@dmmulroy](https://github.com/dmmulroy)).
...
- Fixed Node execution environment commands through legacy WSL `bash.exe` to pass scripts over stdin so shell variables expand in the target bash ([#5893](https://github.com/crow/pi/issues/5893)).

## [0.79.8] - 2026-06-19

### Added

- Added `@crow/pi-agent-core/base` for bundlers that want to pair the agent core with selective `@crow/pi-ai/base` provider registration ([#5348](https://github.com/crow/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
...
- Fixed late tool progress callbacks after tool settlement to be ignored instead of emitting stale `tool_execution_update` events ([#5573](https://github.com/crow/pi/issues/5573)).
...
- Fixed tool-call preflight to stop preparing sibling tool calls after the run is aborted ([#4276](https://github.com/crow/pi/issues/4276)).
- Fixed tail truncation for oversized single-line output that ends with a trailing newline ([#4715](https://github.com/crow/pi/issues/4715)).
- Fixed Windows Node execution environment command spawns to hide helper console windows from background processes ([#4699](https://github.com/crow/pi/issues/4699)).
